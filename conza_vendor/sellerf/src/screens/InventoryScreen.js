...
import {
  ...
  TextInput, Image, Switch, Dimensions,
} from 'react-native';
...
const DUMMY_MATERIALS = [
  {
    ...
    active: true,
    lowStock: true,
    description: 'High quality OPC 53 grade cement',
  },
  {
    id: '2',
    ...
    active: true,
    lowStock: false,
    description: 'TMT Fe-500 grade steel rods',
  },
  {
    id: '3',
  ...
  },
  {
    ...
    active: true,
    lowStock: false,
    description: 'Manufactured sand for plastering',
  },
  {
    id: '6',
    ...
    active: false,
    lowStock: true,
    description: 'Crushed granite aggregate 20mm',
  },
];
...
// ── Product Card ──────────────────────────────────────────────────────────────
const ProductCard = ({ item, onToggleStatus }) => {
  const stockColor =
    item.stock === 0 ? colors.red   :
    item.lowStock    ? colors.orange : colors.green;

  const stockBg =
    item.stock === 0 ? colors.redSoft    :
    item.lowStock    ? colors.orangeSoft : colors.greenSoft;
  ...
  const catStyle = CATEGORY_COLORS[item.category] || { bg: colors.surfaceElevated, color: colors.textMuted };
      ...
      <View style={styles.imageBox}>
        {item.image ? (
          ...
          <View style={[styles.imagePlaceholder, { backgroundColor: catStyle.bg }]}>
            ...
            <Text style={[styles.placeholderCategory, { color: catStyle.color }]}>
              {item.category}
            </Text>
          </View>
        )}
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statValue}>
              ₹{(item.price * item.sold).toLocaleString('en-IN')}
            </Text>
...
const ComingSoon = () => (
  <View style={styles.comingSoon}>
    ...
    <Text style={styles.comingSoonTitle}>Rental Inventory</Text>
...
const InventoryScreen = ({ navigation }) => {
  ...
  const [items, setItems]         = useState(DUMMY_MATERIALS);
    ...
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>
          {mode === 'materials' ? 'Inventory' : 'Equipment'}
        </Text>
        <View style={styles.headerRight}>
          <ModeToggle />
          {mode === 'materials' && (
  <TouchableOpacity activeOpacity={0.85} onPress={() => navigation.navigate('AddProduct')}>
    <LinearGradient
      colors={[colors.gradientStart, colors.gradientEnd]}
      start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
      style={styles.addBtn}
      ...
      {mode === 'rental' ? <ComingSoon /> : (
        <>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ProductCard item={item} onToggleStatus={handleToggleStatus} />
            )}
          />
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  // Tabs
  tabsRow:          { flexDirection: 'row', backgroundColor: colors.surface, paddingHorizontal: 14, paddingVertical: 10, gap: 8, borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  // Price + Stock row
  priceStockRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 14 },
  ...
  comingSoonTitle: { fontSize: 20, fontWeight: '800', color: colors.textPrimary },
  comingSoonSub:   { fontSize: 14, color: colors.textMuted, fontWeight: '500' },
});
