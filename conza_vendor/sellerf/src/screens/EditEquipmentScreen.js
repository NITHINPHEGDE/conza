...
const EditEquipmentScreen = ({ navigation, route }) => {
  ...
  const [totalUnits, setTotalUnits] = useState(item?.stock != null ? String(item.stock) : '');
  const [sku, setSku] = useState(item?.sku || '');
  const [description, setDescription] = useState(item?.description || '');
  const [minDays, setMinDays] = useState(item?.minRentalDays != null ? String(item.minRentalDays) : '');
  const [maxDays, setMaxDays] = useState(item?.maxDays != null ? String(item.maxDays) : '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Equipment Updated', `${name} has been updated.`, [
          ...
          <View style={styles.rowInputs}>
            <View style={styles.halfField}>
              ...
              <TextInput style={styles.input} placeholder="1"
                placeholderTextColor={colors.textMuted} value={minDays}
                onChangeText={setMinDays} keyboardType="numeric" />
            </View>
            <View style={styles.halfField}>
              <Text style={styles.fieldLabel}>Max Rental Days</Text>
              <TextInput style={styles.input} placeholder="30"
                placeholderTextColor={colors.textMuted} value={maxDays}
                onChangeText={setMaxDays} keyboardType="numeric" />
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Field label="Total Units Available *">
            <TextInput style={styles.input} placeholder="e.g. 3"
              placeholderTextColor={colors.textMuted} value={totalUnits}
              onChangeText={setTotalUnits} keyboardType="numeric" />
          </Field>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.rowInputs}>
            <View style={styles.halfField}>
              ...
              <TextInput style={styles.input} placeholder="e.g. 250 kg"
                placeholderTextColor={colors.textMuted} value={weight} onChangeText={setWeight} />
            </View>
            <View style={styles.halfField}>
              <Text style={styles.fieldLabel}>Dimensions</Text>
              <TextInput style={styles.input} placeholder="e.g. 1.2m × 0.8m"
                placeholderTextColor={colors.textMuted} value={dimensions} onChangeText={setDimensions} />
            </View>
          </View>
        </View>
