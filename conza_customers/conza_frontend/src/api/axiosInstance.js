...
const rawBaseUrl =
  (process.env.EXPO_PUBLIC_API_URL || 'https://conza-production-b541.up.railway.app/api').trim();

const trimmedBase = rawBaseUrl.replace(/\/+$/, '');
const BASE_URL = trimmedBase.endsWith('/api') ? trimmedBase : `${trimmedBase}/api`;
