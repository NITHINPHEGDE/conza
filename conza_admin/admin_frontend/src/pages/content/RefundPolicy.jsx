...
export default function RefundPolicy() {
  ...
  const load = useCallback(async (appTarget) => {
    ...
    try {
      ...
      setTitles((prev) => ({ ...prev, [appTarget]: refund?.title || 'Refund Policy' }))
      setBodies((prev) => ({ ...prev, [appTarget]: refund?.content || '' }))
    } catch (err) {
      addToast(err.message || 'Failed to load refund policy.', 'error')
    } finally {
      setLoading(false)
    }
  }, [addToast])
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await saveLegal('refund', activeTab, {
        title: titles[activeTab] || 'Refund Policy',
        content: bodies[activeTab] || '',
      })
      addToast('Refund Policy saved.', 'success')
    } catch (err) {
      addToast(err.message || 'Failed to save refund policy.', 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: 'Content' }, { label: 'Legal' }, { label: 'Refund Policy' }]} />
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-textPrimary">Refund Policy</h1>
      ...
      <div className="bg-surface rounded-xl border border-border p-6">
        <div className="flex items-center gap-3 mb-4">
          <RotateCcw size={20} className="text-accentAmber" />
          <h3 className="font-semibold text-textPrimary">
            Edit Refund Policy ({categories.find((c) => c.key === activeTab)?.label})
          </h3>
        </div>
