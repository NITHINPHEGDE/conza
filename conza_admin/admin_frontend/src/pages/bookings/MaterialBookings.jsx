...
export default function MaterialBookings() {
  const { getBookingsByType } = useBookingStore()
  const [filters, setFilters] = useState({ status: 'all', search: '' })
  const bookings = getBookingsByType('materials')
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: 'Operations' }, { label: 'Bookings' }, { label: 'Materials' }]} />
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-textPrimary flex items-center gap-2">
          <Package size={24} className="text-accentAmber" />
          Material Bookings
        </h1>
      </div>
